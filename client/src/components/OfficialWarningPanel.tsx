    ...
    <section className={`official-warning-panel ${active ? "has-active-warning" : "no-active-warning"}`}>
      ...
      <aside className="warning-side">
        ...
        <div className="schedule-readiness">
          <AlertTriangle />
          <span><b>{automation.data?.configured ? "Automatic refresh configured" : "Automatic refresh ready after deployment"}</b>{automation.data?.jobs?.map(job => job.cronExpression).join(" · ") || "forecast 4× daily · verification daily"} UTC</span>
        </div>
      </aside>
    </section>
  );
